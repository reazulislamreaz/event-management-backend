...
const computeNextAutoGenerateDate = (input: IRepeatConfigInput): Date | null => {
  const frequency =
    input.repeatFunction ??
    (input.repeatUnit === RepeatUnit.Day
      ? RepeatFrequency.Daily
      : input.repeatUnit === RepeatUnit.Week
        ? RepeatFrequency.Weekly
        : input.repeatUnit === RepeatUnit.Month
          ? RepeatFrequency.Monthly
          : input.repeatUnit === RepeatUnit.Year
            ? RepeatFrequency.Yearly
            : RepeatFrequency.DontRepeat);
  if (frequency === RepeatFrequency.DontRepeat) return null;

  const interval = Math.max(1, input.repeatEvery ?? 1);
  ...
  switch (frequency) {
    ...
    case RepeatFrequency.Custom: {
      const unit = input.repeatUnit ?? RepeatUnit.Day;
...
const repeatConfigFields = (input: IRepeatConfigInput) => ({
  ...
  interval: input.repeatEvery ?? 1,
  unit: input.repeatUnit ?? null,
  weekDays: input.daysOfWeek ?? [],
  monthDay: input.dayOfMonth ?? null,
  monthWeek: input.weekOfMonth ?? null,
  monthWeekDay: input.weekDay ?? null,
  endType: input.untilDate ? RepeatEndType.OnDate : RepeatEndType.Never,
...
const createEvent = async (creatorId: string, payload: ICreateEventPayload) => {
  ...
  const repeatFrequency =
    repeatConfig?.repeatFunction ??
    (repeatConfig?.repeatUnit === RepeatUnit.Day
      ? RepeatFrequency.Daily
      : repeatConfig?.repeatUnit === RepeatUnit.Week
        ? RepeatFrequency.Weekly
        : repeatConfig?.repeatUnit === RepeatUnit.Month
          ? RepeatFrequency.Monthly
          : repeatConfig?.repeatUnit === RepeatUnit.Year
            ? RepeatFrequency.Yearly
            : RepeatFrequency.DontRepeat);
...
const getEventById = async (id: string) => {
  ...
  return {
    ...event,
    eventSessions: event.eventSessions.map(s => withEventSessionCostEstimation(s)),
  };
};
...
const getEventSessions = async (eventId: string) => {
  ...
  return rows.map(s => withEventSessionCostEstimation(s));
};
...
const verifyEventSessionById = async (eventId: string, eventSessionId: string) => {
  ...
  if (row.status !== SessionStatus.Unverified) {
    return withEventSessionCostEstimation(row);
  }
  const updated = await database.eventSession.update({
    where: { id: row.id },
    data: { status: SessionStatus.Published },

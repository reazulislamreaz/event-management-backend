...
const computeNextAutoGenerateDate = (input: IRepeatConfigInput): Date | null => {
  const frequency = input.frequency ?? RepeatFrequency.DontRepeat;
  ...
  switch (frequency) {
    ...
    case RepeatFrequency.Custom: {
      const unit = input.unit ?? RepeatUnit.Day;
      if (unit === RepeatUnit.Week) return addDays(base, interval * 7);
      if (unit === RepeatUnit.Month) return addMonths(base, interval);
      if (unit === RepeatUnit.Year) return addYears(base, interval);
      return addDays(base, interval);
    }
    default:
      return null;
...
const verifyEventSessionById = async (eventId: string, eventSessionId: string) => {
  const row = await database.eventSession.findFirst({
  ...
  if (row.status !== SessionStatus.Unverified) {
    return row;
  }
  return database.eventSession.update({
    where: { id: row.id },
